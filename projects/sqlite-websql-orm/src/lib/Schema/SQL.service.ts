...
import { DeleteOption, UpdateOption, SelectOption, CreateOption } from './SQLQueriesInterfaces';
 ...
 * }
 */
  getCreateTableSql(options: CreateOption): string {
    ...
    for (const item in options.tableSchema) {
      if (item) {
        const field = options.tableSchema[item].name;
        const type = options.tableSchema[item].type;

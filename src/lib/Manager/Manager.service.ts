...
export class Manager implements ManagerInterface {
  ...
  async flush(entityInstance: EntityInterface = null) {
    ...
    return new Promise((resolve, reject) => {
      this.transaction(this.getConnector().connection , async (transactionObject) => {
        const promises: any[] = [];
        ...
        currentTaskQueue.forEach(async (action: TaskInterface) => {
          ...
          const repositoryInstance = RepositoryStore.getRepositoryInstance(entityName, this);

          switch (action.type) {
            case 'persist':
              promises.push({
                promise: repositoryInstance.use(transactionObject).insert(action.entity, {hash: action.hash}),
              });
            break;
            case 'merge':
              promises.push({
                promise: repositoryInstance.use(transactionObject).update(action.entity),
              });
            break;
            case 'remove':
              promises.push({
                promise: repositoryInstance.use(transactionObject).delete(action.entity)
              });
            break;
            case 'removeAll':
              promises.push({
                promise: repositoryInstance.use(transactionObject).delete()
              });
            break;
            default: break;
          }
